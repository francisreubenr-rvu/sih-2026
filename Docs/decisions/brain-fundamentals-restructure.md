...
Target pipeline, taken from the briefing's description of `sightline-v2-foundation@2afd215`. Not present as code on this branch:
...
Verified on this checkout before writing this note: `warden/` is absent. Root `extension/` is absent. `sightline-v2-foundation` is not among the remotes of this clone, so the 91 / 52 split below is the briefing's recorded figure. It was not recomputed here.

## Divergence (master vs Warden line)

| Fact | Master (this checkout, `8b2298e`) | Warden line (`sightline-v2-foundation@2afd215`, as recorded in `Docs/grokbot-briefing.md`) |
|---|---|---|
| `warden/` | Absent. Do not create one to make the briefing true. | Present on that commit. FastAPI on `127.0.0.1:8756`. |
| Root `extension/` | Absent. Shipping extension source is `Prototype/extension/`. | Present on that commit. Side panel v0.4.0. The briefing says that folder is the only build to load on that line, and `Prototype/extension/` is labelled superseded there. |
...
| `PLAN.md` | Opens at Wave 7 (14 September 2026) until this decision's section is prepended. | Side-panel and v4 PII boxes were still unchecked on `2afd215` after the work landed. Trust results files on that commit, not those boxes. |
| Visual system | ARCH-002 HUD. See conflict below. | Pixel tokens in that line's `DESIGN.md` / `extension/pixel.css`. Not in this tree. |
...
**Warden-line tokens**, quoted from `Docs/grokbot-briefing.md` (measured on that branch 13 September 2026; the file is not in this tree):
...
Until Francis picks a side in a later decision, **ARCH-002 remains the chrome on master**. A Warden code merge must not silently replace these tokens, and a wrap pass must not silently adopt the Warden hex set.
...
Ordered. None of these are done by this PR. Do not check them off until the named artifact exists.

| Phase | Work | Honest source |
|---|---|---|
| A | Architecture merge design: how PERCEIVE→STRIP→PLAN→VALIDATE→EXECUTE sits on the master prototype without dropping F17, the protected-scene wire, or the ARCH-002 conflict note. | A decision doc. No code. |
| B | Bring the Warden package and the root side panel onto a **feature branch**, from a real tree: local `sightline-v2-foundation` or cherry-picks of commits that already contain those files. | Do not invent `warden/` or root `extension/` files. If the source commit is absent, stop and say so. |
| C | Reconcile `Prototype/extension/` (master shipping popup) with root `extension/` (Warden-line side panel). Keep historical folders. Label which build loads. | After phase B exists on that feature branch. |
| D | Re-measure PII v3 on the frozen corpus **if and when** strip exists, into a new results file. If F1 falls, report the fall. Do not edit the dataset to improve a score. | `Benchmarks/results/` only. v3/v4 figures in the briefing are that line's record, not a new measurement here. |
| E | Evidence gates only when real results files exist. Re-run `scripts/check_release.py` on the tree you have. Nonzero exit while a mandatory gate fails is correct. | No hand-edited pass. |

## Non-goals for this PR

- No `warden/` code, tests, or `.env`.
- No root `extension/` code.
