...
- Two in-app browser constraints found and documented in `Docs/decisions/face-scale-and-in-app-browser.md`: `tab.playwright.evaluate` runs in a read-only isolated scope with no `fetch` and no view of page-assigned globals, and inline `<script>` blocks did not execute while an external `<script type="module" src>` did. Harnesses must therefore be split into markup plus an external module and must report through DOM attributes.
- Measured the packaged UltraFace RFB-320 detector over one synthetic source image at fourteen square sizes from 48 px to 320 px. All fourteen produced exactly one detection; per-inference times ran 9-20 ms and the matrix finished in 382 ms. Record: `Benchmarks/results/operations-v01/face-scale-v01.json`.
- The previously preserved zero-detection reading at 80 px did not reproduce and is no longer described as a detector blind spot. It remains a single-image local measurement, not a face-detection accuracy benchmark, and does not authorise a real-world redaction coverage claim.
- Website refreshed with the face-scale result, the integrated Earth-observation runs, and the external frozen-screen privacy failure (exact annotated personal data retained on 58 of 100 synthetic screens). A duplicate section introduced during the edit was removed in `25acdf1`; the live page was re-verified in the in-app browser at one `#latest-evidence` section.
- Prototype unit tests: 74 passing. Pages deploy for `25acdf1` succeeded and the live site was verified.
...
- The claimed regression test for the guard did not exist in the repository and the deck's "75 recorded Node tests" was therefore unsupported; the suite ran 74. Added the missing test (a non-observable document must be rejected with `/document or shadow root/`; a missing window with `/attached document/`). Suite size is now 75.
...
- Regenerated `Docs/submission-deck.pptx` (6 slides), `Docs/pitch-deck.pptx` (15 slides), `Docs/sightline-template.potx` and `Docs/pitch-speaker-notes.md`. Both decks parse and slide 10 renders the corrected values.
...
- Prior goal turn made progress: the deck builder was rewritten against the newest evidence and the page-agent observe-target defect was traced to `Prototype/shared/page-agent.mjs`.
- Repaired a duplicated patch. `OBSERVABLE_NODE_TYPES`, `observableRoot` and the two guard throws in `Prototype/shared/page-agent.mjs` had each been inserted twice, which is a hard `SyntaxError` at module load. Collapsed to one copy; `node --check` now passes for `page-agent.mjs`, `main.mjs` and `page-agent.test.mjs`.
- The claimed regression test for that guard did not exist in the repository, so the deck's "75 recorded Node tests" was unsupported and the suite ran 74. Added the missing test: a non-observable document must be rejected with `/document or shadow root/`, and a missing window with `/attached document/`. Suite size is now 75.
- Removed an untraceable metric from the talk deck. Slide 10 previously read "139–141 ms · Capture + protection"; no evidence file in the repository contains that figure. Replaced it with the persisted `captureMs` range 92–108 ms from `Benchmarks/results/prototype-v01-browser.json`, which measures the same capture-and-protection window as the app's "Protected in N ms" message. Slide label, source line and speaker note were all updated.
- Verified the deck's other cited figures against their records: `face-scale-v01.json` (14 sizes, 9.1–20.2 ms per inference) and `webpii-text-v01-summary.json` (`completed_screens_with_exact_retained_pii` 58 of 100). The 3,029 ms first-model-step figure resolves to `observations[1].latencyMsDisplayed` in the 9 September `prototype-v01-browser.json` record.
- Regenerated `Docs/submission-deck.pptx` (6 slides), `Docs/pitch-deck.pptx` (15 slides), `Docs/sightline-template.potx` and `Docs/pitch-speaker-notes.md`. Both decks parse and slide 10 renders the corrected values.
- Resolved once unrestricted execution was available. The 11 September report was regenerated from a real run: 75 tests, 75 passing, 0 failing, with the new guard regression test recorded. `Docs/submission-deck.pdf` (6 pages), `Docs/pitch-deck.pdf` (15 pages) and `Docs/process-documentation.pdf` (7 pages) were re-exported and visually checked against the regenerated sources. The earlier blockage was environmental, not a code defect: nine tests bind `127.0.0.1` and failed with `EPERM ... syscall: 'listen'` under the sandbox, `soffice` could not spawn, `.git` was read-only, and automatic approval review was returning `502 Bad Gateway` from the local Ollama guardian ("Guardian did not call submit_guardian_decision") on port 11434.
- Restored `Benchmarks/results/prototype-unit-tests.txt` to its committed 37-test content so a sandbox-only 66/75 run is not left in the tree as if it were evidence.
- Still open from earlier: native extension validation, broader privacy and task datasets, client resource budgets, full-flow latency (G11 remains failed and must not be weakened), human/domain validation, and final packaging.
