...
const token=process.env.DHRISTI_TOKEN||readFileSync(tokenFile,'utf8').trim();
const port=Number(process.env.PORT||9041);
const host=process.env.HOST||'127.0.0.1';
if(host!=='127.0.0.1' && (!process.env.DHRISTI_TOKEN||!process.env.PUBLIC_ORIGIN?.startsWith('https://'))) throw new Error('Public hosting requires DHRISTI_TOKEN and an HTTPS PUBLIC_ORIGIN');
