...
async function prepareDraft(r){
 ...
 const reference=r.vault.issue({value:doc.getElementById('local-email').textContent,kind:'email',target:r.target,revision:scene.revision});r.prepared=localReferenceRequestSchema.parse({scheme:'dhristi-local-references-v1',task:'prepare-report-contact',scene,fields:[{id:'f0',label:'Report contact',...reference,empty:true,rect:r.bounds}]});freshDraft(r);
