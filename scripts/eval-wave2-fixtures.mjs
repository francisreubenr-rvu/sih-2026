/**
 * Score held-out synthetic PII/redaction/utility fixtures.
...
const fixturePath = join(root, 'Benchmarks/datasets/wave2-heldout-pii-fixtures.json');
const argv = process.argv.slice(2);
const jsonFlag = argv.indexOf('--json');
const jsonOut = jsonFlag >= 0
  ? argv[jsonFlag + 1]
  : join(root, 'Benchmarks/results/wave2-pii-redaction-utility-v01.json');
...
const summary = {
  name: 'wave2-pii-redaction-utility',
  version: 'v01',
  ...
  official_score: null,
  scoreReason: 'Held-out synthetic fixtures only. Not WebPII re-score; not organizer saturation. Latency gate remains fail.',
  cases,
  notes: [
