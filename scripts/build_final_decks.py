...
def submission():
 ...
 s=p.slides[3]
 for y,label,body in [(1.55,'Analysis of the feasibility of the idea','Local browser + optional Qwen; 119 Node tests pass (measured 2026-09-15).\nPrivacy-only + Score heuristic; Chromium protect loop harness-verified.'),(3.12,'Potential challenges and risks','Real model step: 3,029 ms; full-flow <200 ms target fails (G11).\nFace-only CV; WebPII text retained PII on 58/100; toolbar needs human.'),(4.7,'Strategies for overcoming these challenges','Held-out fixtures + privacy-only path + detector cache (Wave4).\nFail closed on sanitize; expiry/cancel recovery evidenced; judge checklist.')]:
...
def talk():
 ...
 text(s,'119 recorded Node tests pass (2026-09-15). The <200 ms full-flow target does not pass.',.63,5.89,12,.47,23,CAUTION,True)
...
lines=['# Dhristi — 15-slide speaker notes','',f'Planned talk slots: {total} seconds ({total//60}m {total%60}s). Approximately 12 minutes; not a measured rehearsal. Allow operator time on slide 9. Sources and limitations are retained in the PPTX notes.','',
 ...
 '', '## Evidence discipline','',
 'Every timing on these slides is a single local observation on Apple M1 Pro / 16 GB, not a p95, dataset accuracy or cross-browser certification. The 9-20 ms face-inference range comes from the 11 September face-scale matrix; the 92-108 ms capture-and-protection range and the 3,029 ms first-model-step figure come from the 9 September v0.1 record. One hundred nineteen tests is the recorded count as of 2026-09-15 (`cd Prototype && npm test`); replace only with a newly measured count.',
